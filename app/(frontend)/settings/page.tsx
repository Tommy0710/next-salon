...
export default function SettingsPage() {
    ...
    // Ví dụ: const { user } = useAuth(); const currentUserIsAdmin = user?.role?.isAdmin;
    const currentUserIsAdmin = false; // Đang để mặc định false (Staff) để test giao diện Staff
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    {openTabs.qrcode && (
                        <div className="px-6 pb-6 border-t border-gray-200 pt-6">
                            ...
                            {visibleQRCodes && visibleQRCodes.length > 0 ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                                    {visibleQRCodes.map((qr, index) => (
                                ...
                                <p className="text-sm text-gray-500 mb-6">Chưa có mã QR nào (hoặc không có quyền truy cập).</p>
                            )}
